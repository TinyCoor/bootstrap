...
align_of_intrinsic::align_of_intrinsic(compiler::module *module, compiler::block *parent_scope,
    compiler::argument_list *args)
        : intrinsic(module, parent_scope, args)
{

}
std::string_view align_of_intrinsic::name() const
{
    return "align_of";
}
...
bool align_of_intrinsic::on_fold(session &session, fold_result_t &result)
{
    auto args = arguments()->elements();
    if (args.empty() || args.size() > 1) {
        session.error(this, "P091", "align_of expects a single argument.", location());
        return false;
    }
    ...
    result.element = session.builder().make_integer(parent_scope(), arg_type->alignment());
    return true;
}
bool align_of_intrinsic::on_infer_type(const session &session, type_inference_result_t &result)
{
