...
variant_t evaluator::evaluate(result& result, const ast_node_shared_ptr& node)
{
	...
	switch (node->token) {
		...
		case ast_node_t::identifier: {
			...
			auto value = symbol_table_->get(identifier.value);
			if (value == nullptr && symbol_table_->missing_is_error()) {
...
bool evaluator::transform_identifier_node(result& result, const ast_node_shared_ptr& node) {
	...
	address_node->value = numeric_literal_t {compiler_->address()};
...
symbol_table* evaluator::symbol_table() {
	return symbol_table_;
}

void evaluator::symbol_table(class symbol_table* value) {
	symbol_table_ = value;
}
