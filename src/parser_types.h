...
#ifndef BOOTSTRAP__PARSER_TYPES_H_
#define BOOTSTRAP__PARSER_TYPES_H_
...
#define CASE_STR(a) case a: return #a
		switch (value) {
			CASE_STR(radix_numeric_literal);
			CASE_STR(numeric_literal);
			CASE_STR(boolean_literal);
			CASE_STR(identifier);
			CASE_STR(string_literal);
			CASE_STR(char_literal);
			CASE_STR(operator_literal);
			CASE_STR(comment_literal);
			CASE_STR(directive);
			CASE_STR(label);
			CASE_STR(dup_literal);
			CASE_STR(variadic);
			CASE_STR(any);
		}
		return "unknown";
...
struct radix_numeric_literal_t {
	...
	friend std::ostream &operator<<(std::ostream &stream, const radix_numeric_literal_t &lit) {
		switch (lit.radix) {
			case 2: stream << fmt::format("%{0}", lit.value);
				break;
			case 8: stream << fmt::format("@{0}", lit.value);
				break;
			case 16: stream << fmt::format("${0}", lit.value);
				break;
			default: stream << fmt::format("{0}", lit.value);
				break;
		}
		return stream;
	}
...
typedef std::variant<radix_numeric_literal_t, numeric_literal_t, boolean_literal_t,
					 identifier_t, string_literal_t, char_literal_t, operator_t, comment_t, label_t, dup_literal_t>
	variant_t;
...
struct ast_node_t {
	...
	void serialize(std::ostream &stream) {
		switch (token) {
			case assignment: stream << ":=";
				break;
			case branch: lhs->serialize(stream);
				if (rhs!=nullptr)
			...
			case expression: stream << "(";
				for (const auto &child: children)
					child->serialize(stream);
				stream << ")";
				break;
			case binary_op: lhs->serialize(stream);
				stream << std::get<operator_t>(value);
				rhs->serialize(stream);
				break;
			case unary_op: stream << std::get<operator_t>(value);
				rhs->serialize(stream);
				break;
			case statement:
			case label:
			case identifier: stream << std::get<identifier_t>(value);
				break;
			case address:
			case string_literal:
			case number_literal:
			case boolean_literal:
			case character_literal: stream << std::get<string_literal_t>(value);
				break;
			case comment: stream << std::get<comment_t>(value);
				break;
			case null_literal: stream << "null";
				break;
			case uninitialized_literal: stream << "?";
				break;
		}
	}
...
#endif //BOOTSTRAP__PARSER_TYPES_H_
